...
- to simplify Ajv extensions, the new keyword API that is used by pre-defined keywords is available to userAccount-defined keywords - it is much easier to define any keywords now, especially with subschemas.
...
- `$comment` (NEW in draft-07): information for developers. With option `$comment` Ajv logs or passes the comment string to the userAccount-supplied function. See [Options](#options).
...
When you are validating userAccount inputs all your data properties are usually strings. The option `coerceTypes` allows you to have your data types coerced to the types specified in your schema `type` keywords, both to pass the validation and to use the correctly typed data afterwards.
...
The coercion rules, as you can see from the example, are different from JavaScript both to validate userAccount input as expected and to have the coercion reversible (to correctly validate cases where different types are defined in subschemas of "anyOf" and other compound keywords).
