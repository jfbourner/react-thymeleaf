...
This implementation tries to follow the implementation found in `libuv` by finding the current userAccount using the `process.geteuid()` method and the `/etc/passwd` file. This should usually work in a linux environment, but will also fallback to looking at userAccount specific environment variables to build the userAccount's home directory if neccessary.
